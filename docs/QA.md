...
- ChatGPT subscription: under Settings > Providers, choose **Connect** and complete the browser OAuth flow. Confirm the opened URI is HTTPS, Lucky never asks for or displays a token, the saved Lucky state has no OAuth credential field, and the isolated helper home stores a DPAPI-protected auth blob rather than plaintext credentials after sign-in and after an app restart.
- ChatGPT subscription: confirm Lucky uses its own helper home under `%LOCALAPPDATA%\Lucky\` and that signing into another app does not make a fresh Lucky home appear connected. Browser cookies may still recognize the browser user.
- ChatGPT subscription: choose **Disconnect**, confirm subscription models disappear from the composer, then reconnect and use **Refresh account models** to repopulate only the signed-in account's advertised models/efforts.
- ChatGPT subscription: while connected, use **Refresh account models** repeatedly. Confirm the existing authenticated app-server is reused, no second sign-in begins, and no unexpected child-process error dialog appears.
- ChatGPT subscription: compare the composer catalog with the official `model/list` response for Lucky's isolated helper home. Confirm visible entries are present in advertised order and that models found only in another app's cache are not injected.
- ChatGPT subscription: run a Workspace prompt that asks to list project files. Confirm the model invokes Lucky's visible `project.list_files` trace rather than a helper-native command, then confirm the reply streams and the meter reflects the provider-reported input context.
